...
func getAnswer(lines []string) int {
	...
	grid = processPointsDistances(grid, points)
	//printGrid(grid)

	output := getLargestArea(grid)
	fmt.Printf("Output: %d\n", output)
...
func processPointsDistances(grid Grid, points []Point) Grid {

	for y := 0; y < len(grid); y++ {
		for x := 0; x < len(grid[y]); x++ {
			c := grid[y][x]
			if c == "??" {
				grid[y][x] = findLowestDistance(x, y, points)
			}
		}
	}
...
func findLowestDistance(x int, y int, points []Point) string {
	pointToDistance := make(map[string]int)
	distanceToPoint := make(map[int][]string)
	for _, p := range points {
		dist := int(math.Abs(float64(x-p.X)) + math.Abs(float64(y-p.Y)))
		pointToDistance[p.Name] = dist
		distanceToPoint[dist] = append(distanceToPoint[dist], p.Name)
	}
	//printDistances(pointToDistance)

	point := "??"
	minDistance := math.MaxUint32
	for k, v := range pointToDistance {
		if v < minDistance {
			minDistance = v
			point = k
		}
	...
	if len(distanceToPoint[minDistance]) >= 2 {
		return ".."
	}

	if len(distanceToPoint[minDistance]) == 1 {
