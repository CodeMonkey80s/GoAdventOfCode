...
func Test_sumOfAllTheCalibrationValues_part1(t *testing.T) {
	...
	for _, tc := range testCasesPart1 {
		...
		t.Run(label, func(t *testing.T) {
			output := sumOfAllTheCalibrationValues_part1(tc.Input)
			if output != tc.Output {
...
func Test_sumOfAllTheCalibrationValues_part2(t *testing.T) {
	var label string
	for _, tc := range testCasesPart2 {
		label = fmt.Sprintf("Case: Input: %v Output: %v\n", len(tc.Input), tc.Output)
		t.Run(label, func(t *testing.T) {
			output := sumOfAllTheCalibrationValues_part2(tc.Input)
			if output != tc.Output {
				t.Errorf("Expected output to be %v but we got %v", tc.Output, output)
			}
		})
